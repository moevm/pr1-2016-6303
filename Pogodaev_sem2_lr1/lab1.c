...
int main() {
  ...
  for (i=0;i<N;i++)//заполнение массива числами
    ...
    timeBeg1 = clock(); //время начала бинарного поиска
    int *ptrItem=(int*) bsearch( &key, arr, 100, sizeof(int), funccmp);
    timeEnd1 = clock(); //время завершения бинарного поиска

    if (ptrItem==NULL) {  //вывод результата поиска
    printf("doesn't exist\n");}
    else {
      printf("exists\n");}

    printf("%d\n",timeEnd1-timeBeg1);//вывод времени работы бинарного поиска

    int l = 0; //поиск перебором
    timeBeg2 = clock();//время начала поиска перебором
    for (i=0;i<N;i++)
    {
        if (arr[i]==key)
        {
    ...
    timeEnd2 = clock();//время завершения поиска перебором
