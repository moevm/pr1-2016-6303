...
void push(MusicalComposition *head) // добавляет element  в конец списка musical_composition_list
{
...
void removeEl(MusicalComposition *head)// удаляет элемент element списка, у которого значение name равно значению  name_for_remove
{
    char name_for_remove[81];
    int amount = count(head);//возвращает количество элементов списка
